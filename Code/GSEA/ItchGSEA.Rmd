...
```{r Function to Read in aDRG GSEA data, include = T, message = F, warning = F, echo = T}
...
```{r Function to Wrangle aDRG GSEA data, include = T, message = F, warning = F, echo = T}
...
```{r Function to Read in eDRG GSEA data, include = T, message = F, warning = F, echo = T}
...
```{r Function to Wrangle eDRG GSEA data, include = T, message = F, warning = F, echo = T}
...
```{r Read in aDRG DEA Results, include = F, message = F, warning = F, echo = F}
...
```{r Read in aDRG GSEA Results, include = T, message = F, warning = F, echo = T}
...
```{r Join aDRG GSEA Results, include = T, message = F, warning = F, echo = T}
for (i in 1:length(aDRG_categories)) {
...
```{r Plot aDRG GO GSEA results, include = T, message = F, warning = F, echo = T, fig.width=10, fig.width=14}
aDRG_GO_results |>
...
```{r Read in eDRG DEA Results, include = F, warning = F, message = F, echo = F}
e13_DRG <- read.csv(
...
p10_DRG <- read.csv(
  ...
  header = T)
```
```{r Write eDRG Results, include = F, warning = F, message = F, echo = F}
write.table(
  e13_DRG |>
...
```{r Read in eDRG GO and Pathway Results, include = T, warning = F, message = F, echo = T}
ages <- c('e13', 'p0', 'p10')
...
```{r Wrangle all eDRG GSEA results, include = T, message = F, warning = F, echo = T}
for (i in 1:length(ages)) {
  for (j in 1:length(categories)) {
...
```{r Join all eDRG GSEA results, include = T, message = F, warning = F, echo = T}
...
```{r Plot eDRG Results, include = T, warning = F, message = F, echo = T, fig.height=11, fig.width=15}
...
```{r Plot eDRG PANTHER Results, include = T, warning = F, message = F, echo = T, fig.height=8,fig.width=14}
...
```{r Retro Paper GO fig, include = T, message = F, warning = F, echo = T, fig.height=7, fig.width=12}
eDRG_GO_results |>
...
```{r Current Paper GO fig, include = T, message = F, warning = F, echo = T, fig.height=7, fig.width=12}
eDRG_GO_results |>
  dplyr::filter(devStage == 'e13', GO_Type == 'BP') |>
  dplyr::arrange(FDR, desc(fold.Enrichment)) |> 
  dplyr::mutate(sig = dplyr::case_when(FDR <= 0.001 ~ "***",
